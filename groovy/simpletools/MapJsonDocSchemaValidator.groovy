 ...
 *
 *     Description
 *         This parses a very simple JSON schema format and then validates JSON document against it. Note that
 *         this does not make use of any specific JSON parser or external JSON model. It represents JSON as
 *         Map<String, Object> and JSON arrays as List<Object>. So however you parse your JSON, it must be
 *         represented as these standard Java structures for this to be used.
 *
 * COPYRIGHTS
...
class MapJsonDocSchemaValidator implements MapJsonSchemaConst {
    ...
    private static class MapObject {
        ...
        private static boolean verifyKey( String mapKey, Map<String, Object> source ) {
            ...
            if ( parts.length >= 2 && ( parts[ 1 ] != '0' && parts[ 1 ] != '1' ) && parts[ 1 ] != '?' ) {
                ...
                throw new RuntimeException( "Bad key format! [$mapKey] Should be 'name' or 'name_0' or 'name_1'. ${ source }" )
            }
     ...
     */
    void validate( Map<String, Object> toValidate ) {

        if ( toValidate == null ) {

            throw new RuntimeException( "Input to validate is null!" )
        }
    ...
    private static void validateBoolean( Object sourceValue, Object errorSource ) {
        if ( !Boolean.class.isAssignableFrom( sourceValue.class ) && !boolean.class.isAssignableFrom( sourceValue.class ) ) {
            throw new RuntimeException( "Value '${ sourceValue }' must be a boolean! ${ errorSource }" )
        }
    }
    ...
    private static void validateString( String validValue, Object sourceValue, Object errorSource ) {
        ...
        if ( validValue.startsWith( REGEXP ) || validValue.startsWith( ENUMERATION ) ) {
            ...
            if ( !sourceValue.toString().matches( regExp ) ) {
                ...
                throw new RuntimeException( "Value '${ sourceValue }' does not match regular expression '${ regExp }'! ${ errorSource }" )
            }
        }
        else {
            ...
            if ( sourceValue.toString() != validValue ) {
                ...
                throw new RuntimeException( "Found '$sourceValue'. Expected '$validValue'! $errorSource" )
            }
        }
    }
        ...
        if ( equals ) {
            ...
            if ( from != null && to == null ) {
                ...
                if ( value >= fromD ) {
                    ...
                    throw new RuntimeException( "Value ($value) must be >= $fromD! $errorSource" )
                }
            }
                ...
                if ( value <= toD ) {
                    ...
                    throw new RuntimeException( "Value ($value) must be <= $toD! $errorSource" )
                }
            }
            else {
                ...
                if ( value >= fromD && value <= toD ) {
                    ...
                    throw new RuntimeException( "Value ($value) must be >= $fromD && <= $toD $errorSource" )
                }
            }
        }
        else {
    ...
    private void validateMap( Map<String, Object> validStructure, Map<String, Object> toValidate ) {
        ...
        toValidate.each { String key, Object value ->
            ...
            if ( doValidate ) {
                ...
                if ( !validMO.keySet().contains( key ) ) {
                    ...
                    throw new RuntimeException( "Entry '${ key }' is not valid! $toValidate" )
                }
                ...
                if ( validMO.isRequired( key ) && sourceValue == null ) {
                    ...
                    throw new RuntimeException( "'${ key }' is required! $toValidate" )
                }
        ...
        validMO.keySet().each { String key ->
            ...
            if ( validMO.isRequired( key ) && !toValidate.keySet().contains( key ) ) {
                ...
                throw new RuntimeException( "Missing entry for required '$key'! $toValidate" )
            }
        }
